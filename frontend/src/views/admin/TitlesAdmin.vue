      ...
      <section class="panel panel-left">
        <div class="toolbar">
          <div class="actions">
            ...
            <el-button :disabled="!hasChecked" @click="clearChecked">
              {{ t("titles.actions.clearSelection") }}
            </el-button>
          </div>
          <el-input
            v-model="keyword"
              ...
              <div class="tree-right">
                <div class="tree-actions">
                  <el-button
                    ...
                    size="small"
                    class="icon-button"
                    :title="t('titles.actions.addChild')"
                    :aria-label="t('titles.actions.addChild')"
        ...
        <div class="panel-header">
          <div class="panel-title">
            ...
            <el-tag v-if="formMode !== 'empty'" size="small" class="panel-tag">
              {{ t("titles.levelTag", { level: formLevel }) }}
            </el-tag>
          </div>
        </div>

        <el-empty v-if="formMode === 'empty'" :description="t('titles.form.empty')" />
        <div v-else class="form-card">
          <el-form ref="formRef" :model="formState" :rules="formRules" label-width="120px">
            <div class="form-section">
              <div class="form-section-title">
                {{ t("titles.form.section.basic") }}
              </div>
              <el-form-item v-if="formParent" :label="t('titles.form.parent')">
                <el-input :model-value="formParent.name" disabled />
              </el-form-item>
              <el-form-item :label="t('titles.form.name')" prop="name" required>
                <el-input v-model="formState.name" />
              </el-form-item>
              <el-form-item :label="t('titles.form.code')" prop="code" required>
                <el-input v-model="formState.code" />
              </el-form-item>
            </div>
            <div class="form-section">
              <div class="form-section-title">
                {{ t("titles.form.section.settings") }}
              </div>
...
const formRules = computed<FormRules>(() => ({
  name: [
    {
      required: true,
      message: t("titles.form.nameRequired"),
      trigger: "blur",
    },
  ],
  code: [
    {
      required: true,
      message: t("titles.form.codeRequired"),
      trigger: "blur",
    },
  ],
}));
...
const formTitle = computed(() => {
  if (formMode.value === "empty") {
    return t("titles.form.title");
  }
  return formMode.value === "edit"
    ? t("titles.dialog.edit")
    : t("titles.dialog.new");
});
...
function buildTree(
  ...
  const sorted = [...titles].sort(sortByOrder);
  return sorted.map((title) => {
    const node: TreeNode = {
      ...
      parent_id: title.parent_id ?? parent?.id ?? null,
      parent_name: parent?.name ?? null,
      level,
      children: [],
    };
    const children = buildTree(title.children ?? [], level + 1, node);
...
function getFallbackIdOnDelete(id: number) {
...
async function refreshTree() {
  ...
  try {
    const result = await listTitleTree();
...
async function submitForm() {
  ...
  try {
    if (formMode.value === "edit" && selectedNode.value) {
      await updateTitle(selectedNode.value.id, {
        name: formState.name,
    ...
    ElMessage.error(
      resolveErrorMessage(error, t("titles.messages.opFailed"), translateTitleDetail),
    );
  }
}

async function confirmDelete(node: TreeNode) {
  const message = t("titles.messages.deleteConfirm", { name: node.name });
  const wasSelected = selectedNode.value?.id === node.id;
  const fallbackId = wasSelected ? getFallbackIdOnDelete(node.id) : null;
  try {
  ...
  try {
    ...
    await refreshTree();
    if (fallbackId) {
      await focusNodeById(fallbackId, "edit");
    }
  } catch (error) {
    const fallback = t("titles.messages.deleteFailed");
    ElMessage.error(resolveErrorMessage(error, fallback, translateTitleDetail));
  }
}
...
async function handleBatchEnable() {
  ...
  batching.value = true;
  try {
    const result = await batchTitles("enable", buildBatchItems("all"));
    ElMessage.success(
      t("titles.messages.batchEnableSuccess", { count: result.updated }),
    );
    await refreshTree();
  } catch (error) {
    ElMessage.error(
      resolveErrorMessage(error, t("titles.messages.batchFailed"), translateTitleDetail),
    );
  } finally {
...
async function handleBatchDisable() {
  ...
  batching.value = true;
  try {
    const result = await batchTitles("disable", buildBatchItems("all"));
    ElMessage.success(
      t("titles.messages.batchDisableSuccess", { count: result.updated }),
    );
    await refreshTree();
  } catch (error) {
    ElMessage.error(
...
async function handleBatchDelete() {
  ...
  try {
    await ElMessageBox.confirm(
      t("titles.messages.batchDeleteConfirm", { count: checkedCount.value }),
      t("common.confirm"),
      { type: "warning" },
    );
  } catch {
  ...
  try {
    const result = await batchTitles("delete", buildBatchItems("primary"));
    ElMessage.success(
    ...
    if (result.skipped > 0 && firstError) {
      const translated = translateTitleDetail(firstError) ?? firstError;
      ElMessage.warning(
        t("titles.messages.batchDeleteSkippedReason", {
          skipped: result.skipped,
          reason: translated,
        }),
      );
...
function translateTitleDetail(detail: string) {
  switch (detail) {
    case "Title code already exists":
      return t("titles.messages.codeExists");
    case "Title code is required":
      return t("titles.messages.codeRequired");
    case "Parent title not found":
      return t("titles.messages.parentNotFound");
    case "Cannot set parent to descendant":
      return t("titles.messages.parentInvalid");
    case "Title must be a leaf":
      return t("titles.messages.leafRequired");
    case "Title not found":
      return t("titles.messages.notFound");
    default:
      return null;
  }
}
